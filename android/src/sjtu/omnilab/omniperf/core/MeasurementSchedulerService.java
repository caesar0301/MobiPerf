...
import sjtu.omnilab.omniperf.core.BatteryCapPowerManager.PowerAwareTask;
...
/**
 * The single scheduler thread that monitors the task queue, runs tasks at their
 * specified times, and finally retrieves and reports results once they finish.
 * 
 * All method invocations on the singleton object are thread-safe.
 */
public class MeasurementSchedulerService extends Service {
    ...
    private final IBinder ibinder = new SchedulerBinder();
    ...
    public class SchedulerBinder extends Binder {
        public MeasurementSchedulerService getService() {
            return MeasurementSchedulerService.this;
        }
    }

    /*
     * Returns a IBinder that contains the instance of the MeasurementScheduler
     * object
     * 
    ...
    public IBinder onBind(Intent intent) {
        Logger.d("Measurement Scheduler Service onBind called");
        return this.ibinder;
    }
    ...
    public void onCreate() {
        Logger.d("Measurement Scheduler Service onCreate called");
        ...
        this.taskQueue = new PriorityBlockingQueue<MeasurementTask>(Config.MAX_TASK_QUEUE_SIZE, new TaskComparator());
        ...
        this.mApp.initNotificationManager();
    }
    ...
    public void handleCheckin(boolean force) {
        ...
        if (!force && mPreferences.getBoolean(Config.PREF_KEY_IS_PAUSE_REQUESTED, true)) {
            sendStringMsg("Skipping checkin - app is paused");
            return;
        }
        if (!force && !mApp.getPowerManager().canScheduleExperiment()) {
            sendStringMsg("Skipping checkin - below battery threshold");
            return;
        }
        /*
         * The CPU can go back to sleep immediately after onReceive() returns.
         * Acquire the wake lock for the new thread here and release the lock
        ...
        try {
            ...
            if (task != null && task.timeFromExecution() <= 0) {
                ...
                if (task.getDescription().priority == MeasurementTask.USER_PRIORITY) {
                    ...
                    // User task can override the power policy. So a different
                    // task wrapper is used.
                    future = measurementExecutor.submit(new UserMeasurementTask(task));
                } else {
                    sendStringMsg("Scheduling task:\n" + task);
                    future = measurementExecutor.submit(new PowerAwareTask(task, mApp));
                }
                ...
                if (newStartTime < desc.endTime.getTime() && (desc.count == MeasurementTask.INFINITE_COUNT || desc.count > 1)) {
            ...
            task = taskQueue.peek();
            if (task != null) {
                long timeFromExecution = Math.max(task.timeFromExecution(), Config.MIN_TIME_BETWEEN_MEASUREMENT_ALARM_MSEC);
                measurementIntentSender = PendingIntent.getBroadcast(this, 0,
                        new UpdateIntent("", UpdateIntent.MEASUREMENT_ACTION),
                        PendingIntent.FLAG_CANCEL_CURRENT);
                mApp.getAlarmManager().set(AlarmManager.RTC_WAKEUP,
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
    	boolean isSchedulerStarted = mPreferences.getBoolean(Config.PREF_KEY_IS_SCHEDULER_STARTED, false);
        Logger.d("Measurement Scheduler Service onStartCommand called, isSchedulerStarted = " + isSchedulerStarted);
        ...
        if (!isSchedulerStarted ) {
            ...
            this.resume();
            /*
             * There is no onStop() for services. The service is only stopped
             * when the user exits the application. So don't worry about setting
             * isSchedulerStarted to false.
             */
            SharedPreferences.Editor editor = mPreferences.edit();
            editor.putBoolean(Config.PREF_KEY_IS_SCHEDULER_STARTED, true);
            editor.commit();
        }
        return START_STICKY;
    }
    ...
    public void onDestroy() {
        Logger.d("Measurement Scheduler Service onDestroy called");
        super.onDestroy();
        cleanUp();
    }
    ...
    public synchronized void setCheckinInterval(long interval) {
        ...
        checkinIntentSender = PendingIntent.getBroadcast(this, 0,
                ...
                PendingIntent.FLAG_CANCEL_CURRENT);
        mApp.getAlarmManager().setRepeating(AlarmManager.RTC_WAKEUP,
    ...
    /**
     * Prevents new tasks from being scheduled. Started task will still run to
     * finish.
     */
    public synchronized void pause() {
        Logger.d("Measurement Scheduler Service pause called");
    ...
    public synchronized void resume() {
        Logger.d("Measurement Scheduler Service resume called");
        sendStringMsg("Scheduler resuming");
        SharedPreferences.Editor editor = mPreferences.edit();
        editor.putBoolean(Config.PREF_KEY_IS_PAUSE_REQUESTED, false);
        editor.commit();
        updateStatus();
    }
    ...
    public synchronized void requestStop() {
        sendStringMsg("Measurement Scheduler stop requested");
        this.notifyAll();
        this.stopForeground(true);
        this.mApp.removeIconFromStatusBar();
        this.stopSelf();
        SharedPreferences.Editor editor = mPreferences.edit();
        editor.putBoolean(Config.PREF_KEY_IS_STOP_REQUESTED, true);
        editor.commit();
    }

    /**
     * Submit a MeasurementTask to the scheduler. Caller of this method can
     * broadcast an intent with MEASUREMENT_ACTION to start the measurement
     * immediately.
     */
    ...
    private void updateFromPreference() {
        Logger.d("Measurement Scheduler Service updateFromPreference called");
        try {
            mApp.getPowerManager().setBatteryThresh(Integer.parseInt(mPreferences.getString(
                    getString(R.string.batteryMinThresPrefKey),
                    String.valueOf(Config.DEFAULT_BATTERY_THRESH_PRECENT))));
            this.setCheckinInterval(Integer.parseInt(mPreferences.getString(
            ...
            Logger.i("Preference set from SharedPreference: " + "checkinInterval=" + checkinIntervalSec
                    + ", minBatThres= " + mApp.getPowerManager().getBatteryThresh());
        } catch (ClassCastException e) {
    ...
    private synchronized void cleanUp() {
        Logger.d("Measurement Scheduler Service cleanUp called");
        this.taskQueue.clear();
        ...
        if (checkinIntentSender != null) {
            ...
            mApp.getAlarmManager().cancel(checkinIntentSender);
        }
        if (checkinRetryIntentSender != null) {
            checkinRetryIntentSender.cancel();
            mApp.getAlarmManager().cancel(checkinRetryIntentSender);
        }
        if (measurementIntentSender != null) {
            measurementIntentSender.cancel();
            mApp.getAlarmManager().cancel(measurementIntentSender);
        }
        persistState();
        this.notifyAll();
        mApp.getPhoneUtils().shutDown();
        ...
        synchronized (this.pendingTasks) {
            try {
                for (MeasurementTask task : this.pendingTasks.keySet()) {
                        ...
                        if (future.isDone()) {
                            try {
                                ...
                                if (!future.isCancelled()) {
                                    ...
                                    finishedTasks.add(this.getFailureResult(
                                ...
                                if (e.getCause() instanceof MeasurementSkippedException) {
                                    // Don't do anything with this - no need to
                                    // report skipped measurements
                                    sendStringMsg("Task skipped - "
                                            + e.getCause().toString() + "\n"
                                            + task);
                                    Logger.i("Task skipped", e.getCause());
                                } else {
                                    // Log the error
                                    sendStringMsg("Task failed - "
                                            + e.getCause().toString() + "\n"
                                            + task);
                                    Logger.e("Task execution failed",
                                            e.getCause());
                                    finishedTasks.add(this.getFailureResult(
                                            task, e.getCause()));
                                }
                            ...
                            /*
                             * If a task has reached its deadline but has not
                             * been run, remove it and report failure
                             */
                            this.pendingTasks.remove(task);
                            future.cancel(true);
                            finishedTasks
                    ...
                    if (future == null) {
                        /*
                         * Tasks that are scheduled after deadline are put into
                         * pendingTasks with a null future.
                         */
                        this.pendingTasks.remove(task);
                ...
                /*
                 * keySet is a synchronized view of the keys. However, changes
                 * during iteration will throw ConcurrentModificationException.
                 * Since we have synchronized all changes to pendingTasks this
                 * should not happen.
                 */
    ...
    private class CheckinTask implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                /*
                 * Executor stops all subsequent execution of a periodic task if
                 * a raised exception is uncaught. We catch all undeclared
                 * exceptions here
                 */
                Logger.e("Unexpected exceptions caught", e);
                if (checkinRetryCnt > Config.MAX_CHECKIN_RETRY_COUNT) {
                    /*
                     * If we have retried more than MAX_CHECKIN_RETRY_COUNT
                     * times upon a checkin failure, we will stop retrying and
                     * wait until the next checkin period
                     */
                    resetCheckin();
                    ...
                    checkinRetryIntentSender = PendingIntent.getBroadcast(
                            MeasurementSchedulerService.this, 0,
                    ...
                    checkinRetryCnt++;
                    checkinRetryIntervalSec = Math.min(Config.MAX_CHECKIN_RETRY_INTERVAL_SEC, checkinRetryIntervalSec * 2);
                }
            } finally {
    ...
    private MeasurementResult getFailureResult(MeasurementTask task,
        ...
        MeasurementResult result = new MeasurementResult(
                mApp.getPhoneUtils().getDeviceInfo().deviceId,
                mApp.getPhoneUtils().getDeviceProperty(), task.getType(),
    ...
    /**
     * A wrapper Callable class that broadcasts intents when the measurement
     * starts and finishes. Needed for activities to monitor the progress of
     * user measurements.
     */
    private class UserMeasurementTask implements Callable<MeasurementResult> {
        ...
        private void broadcastMeasurementStart() {
            ...
            MeasurementSchedulerService.this.sendBroadcast(intent);
        ...
        /**
         * The call() method that broadcast intents before the measurement
         * starts and after the measurement finishes.
         */
        @Override
    ...
    private synchronized void persistState() {
        Logger.d("Measurement Scheduler Service persistState called");
    ...
    private void restoreState() {
        Logger.d("Measurement Scheduler Service restoreState called");
        initializeConsoles();
        restoreStats();
    }
    ...
    private void restoreStats() {
        completedMeasurementCnt = mPreferences.getInt(Config.PREF_KEY_COMPLETED_MEASUREMENTS, 0);
        failedMeasurementCnt = mPreferences.getInt(Config.PREF_KEY_FAILED_MEASUREMENTS, 0);
    }

    private boolean userConsented() {
        boolean consented = mPreferences.getBoolean(Config.PREF_KEY_CONSENTED, false);
    ...
    private void saveConsoleContent(List<String> consoleContent, String prefKey) {
        Logger.d("Measurement Scheduler Service saveConsoleContent for key "
                + prefKey);
        SharedPreferences.Editor editor = mPreferences.edit();
    ...
    private void initializeConsoles() {
        Logger.d("Measurement Scheduler Service initializeConsoles called");
    ...
    private void restoreConsole(List<String> consoleContent, String prefKey) {
        Logger.d("Measurement Scheduler Service restoreConsole for " + prefKey);
        String savedConsole = mPreferences.getString(prefKey, null);
        if (savedConsole != null) {
            ...
            if (items != null) {
                Logger.d("Read " + items.size() + " items from prefkey "
                        + prefKey);
                for (String item : items) {
                    insertStringToConsole(consoleContent, item);
                }
                Logger.d("Restored " + consoleContent.size()
                        + " entries to console " + prefKey);
            }
        }
    }
    ...
    /**
     * Adds a string to the corresponding console depending on whether the
     * result is a user measurement or a system measurement
     */
