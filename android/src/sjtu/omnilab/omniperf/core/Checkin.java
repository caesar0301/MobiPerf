...
public class Checkin {
	...
	public List<MeasurementTask> checkin() throws IOException {
		...
		try {
			...
			Logger.d("POST status: " + status.toString());
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				if (json != null && MeasurementTask.getMeasurementTypes().contains(json.get("type"))) {
					try {
						MeasurementTask task = MeasurementJsonConvertor.makeMeasurementTaskFromJson(json, this.context);
	...
	private String serviceRequest(String url, String jsonString) throws IOException {
		...
		if (!accountSelector.isAnonymous()) {
			synchronized (this) {
				if (authCookie == null) {
					if (!checkGetCookie()) {
						throw new IOException("serviceRequest: No authCookie yet");
					}
				}
			}
		}
		...
		try {
			...
			Logger.e("Encoding exception in serviceRequest: " + e.toString());
		}
		...
		try {
			return client.execute(postMethod, responseHandler);
		} catch (Exception e) {
			Logger.e("Get exception to post request: " + e.toString());
			return null;
		}
	}
