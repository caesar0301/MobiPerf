...
void WriteUserInputLog(char *pszLogInfo, char *pszLogInfo2) {
	...
	if (ofsEventsLog == NULL) {
		...
		sprintf(szLogFileName, "%s/user_input_log_events", gszFileDir);
...
void ReadDeviceKeyDB() {
	...
	FILE * ifs = fopen("/data/data/sjtu.omnilab.omniperf/files/key.db", "r");
	if (ifs == NULL) {
...
void GetMyDeviceType() {
	...
	sprintf(szPropFileName, "%s/prop", gszFileDir);
	char szCmd[512];
